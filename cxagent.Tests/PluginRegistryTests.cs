...
/// <summary>
/// The mutable registry the plugin feature's whole design rests on, and the turn boundary that keeps
/// it correct: loading is refused mid-turn, and unwiring is one ordered operation.
///
...
public class PluginRegistryTests : IDisposable
{
    ...
    /// the whole plugin, not just the colliding tool: a half-loaded plugin is unpredictable from its
    /// manifest.</summary>
    [Fact]
    ...
    /// still refuses, since a half-loaded plugin is unpredictable, proving the injected check goes
    /// through the same all-or-nothing path as the built-in check.</summary>
    [Fact]
