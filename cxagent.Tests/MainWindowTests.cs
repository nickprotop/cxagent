...
public class MainWindowTests
{
    ...
    public void SessionPanel_ShowsTheDefaultCeiling_WhenNothingIsConfigured()
    {
        var window = new MainWindow(SysOfWidth(140),
            ResolvedConfig.ForTesting(new MockLlmProvider(), "Mock")
                .WithCatalog(new ProviderCatalog(Orchestrator: new OrchestratorSettings())),
            Logs());
        window.Build();
    ...
    public void SessionPanel_SaysNoCap_OnlyForAnExplicitZero()
    {
        var window = new MainWindow(SysOfWidth(140),
            ResolvedConfig.ForTesting(new MockLlmProvider(), "Mock")
                .WithCatalog(new ProviderCatalog(Orchestrator: new OrchestratorSettings(MaxTurns: 0))),
            Logs());
        window.Build();
        window.RefreshSessionPanel();
    ...
    {
        var res = ResolvedConfig.ForTesting(new MockLlmProvider(), "Mock").WithContextWindow(200_000);
    ...
    public void SessionPanel_ListsGeneralOnce_WhenConfigAlsoDefinesIt()
    {
        ...
        var window = new MainWindow(SysOfWidth(140),
            ResolvedConfig.ForTesting(new MockLlmProvider(), "Mock")
                .WithCatalog(new ProviderCatalog(AgentTypes: configured)),
            Logs());
        window.Build();
        window.RefreshSessionPanel();
