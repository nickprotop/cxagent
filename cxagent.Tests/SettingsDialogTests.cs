...
public class SettingsDialogTests
{
    ...
    public async Task OrchestratorEdit_ComposesIntoTheSavedSettings()
    {
        ...
        session.UpdateOrchestrator(session.Working.Orchestrator with { MaxConsults = 7, Copilot = true });
        ...
        var result = await task;
        Assert.Equal(7, result!.Orchestrator.MaxConsults);
        Assert.True(result.Orchestrator.Copilot);
        var loaded = ProviderConfigLoader.LoadAndValidate(paths, new Dictionary<string, string>());
        Assert.Equal(7, loaded.Orchestrator.MaxConsults);
        Assert.True(loaded.Orchestrator.Copilot);
    }
    ...
    public void OrchestratorPage_RendersPromptsSeededFromTheWorkingCopy()
    {
        ...
        // asserts the page actually seeded a prompt with the session's real MaxConsults value.
        var settings = OneProvider("first", "m1") with
        {
            Orchestrator = OrchestratorSettings.Unbounded with { MaxConsults = 13 },
        };
        var paths = new AppPaths(MakeTempDir());
