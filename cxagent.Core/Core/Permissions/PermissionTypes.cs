...
///
/// <para><c>Plugin</c> is the load gate itself: may this plugin's whole load set (see
/// <see cref="Plugins.PluginIdentity"/>) run in this session at all — the only boundary Core can
/// enforce on a binary it did not write. It answers that ONE question, once, at load; it is not
/// consulted again per call the way <c>Tool</c> is — a plugin's own declared per-operation gates
/// reuse <c>Tool</c> instead, because that is already the vocabulary for "may this specific
/// capability run", and a plugin's gated tool is exactly that question asked by a different kind of
/// contributor. The plugin supplies the policy behind its own gate; Core only enforces the answer.</para>
///
