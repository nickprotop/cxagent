...
///
/// <para>WHY THIS IS NOT <c>ISessionObserver</c>. Printing UI messages through the session's
/// observer — dozens of <c>ShowSystemMessage</c> call sites, none of them from Core — turns the
/// session's port into a general-purpose message bus, and makes the layer above need a
/// session-shaped object simply to say something to the user. A command reporting "reloaded 3
/// servers" is not the session reporting anything; it is the UI talking to its own surface.</para>
///
