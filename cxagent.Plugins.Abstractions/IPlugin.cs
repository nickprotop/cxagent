...
/// <summary>
/// A bundle of tools and the executor that runs them.
///
...
/// "Unload" would put a promise in the contract that Core cannot keep. What unwiring actually
/// guarantees: the plugin's tools are gone from the registry, the model is never offered them again,
/// and its child processes are dead. The code staying resident afterward costs memory and nothing
/// else.</para>
/// </summary>
public interface IPlugin
{
    ...
    ///
    /// <para>THE RETURNED MANIFEST MUST MATCH THE SIDECAR FILE. Config-time validation reads the
    /// sidecar because it must know a plugin's tool names before any binary loads, and runtime
    /// confirms that promise against this call. A mismatch is a load failure that names the
    /// difference, not a mismatch this method is free to paper over.</para>
    /// </summary>
