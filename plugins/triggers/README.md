...
**Wake on anything but a clock.** This plugin schedules by time and nothing else. To wake when a
command finishes, use the built-in `run_shell` with `background: true`: it starts a command and tells
the agent the exit code when it exits. Note that the report carries the code and the output path and
nothing more — there is no way to compose an instruction that arrives *with* the exit code, so
"if CI failed, fix the test" is the agent's own decision at that moment rather than something the
caller can set up in advance.
